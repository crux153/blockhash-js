...
(function(exports) {
	...
	var blockhash = {
		hash: function(blob, bits, method) {
			return new Promise(function(resolve, reject) {
				bits = bits || 16;
				method = method || 2;

				// create a canvas element for converting image to imageData
				var canvas = document.createElement("canvas"),
					ctx = canvas.getContext("2d");

				var url = URL.createObjectURL(blob),
					img = new Image(); // create image object for calculation of image width and height
				...
				img.addEventListener("load", function() {
					...
					var data = ctx.getImageData(0, 0, width, height); // get imageData from canvas

					try {
						var hash = blockhashData(data, bits, method); // calculate blockhash
						resolve(hash);
